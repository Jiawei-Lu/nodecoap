    ...
    switch (method_flag) {
        ...
        case COAP_PUT:
            ...
            if (pdu->payload_len <= 10) {
                ...
                // rtc_localtime(((int)payload  - 1577836800), &handler_time);
    ...
    switch (method_flag) {
        ...
        case COAP_PUT:
            ...
            if (pdu->payload_len <= 16) {
                ...
                char payload[16] = { 0 };
                puts("received\n");
                memcpy(payload, (char *)pdu->payload, 16);
                // memcpy(payload, (char *)pdu->payload, pdu->payload_len);
                puts("memery allocati11111111111111on\n");
                printf("%s\n",payload);
                req_count = strtoul(payload, NULL, 10);
                // printf("%d\n", req_count);
                puts("have the number\n");
                return gcoap_response(pdu, buf, len, COAP_CODE_CHANGED);
            }
